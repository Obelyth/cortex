 ...
 * Vendored canvas engine — a port of lib/atlas/template.ts: same ring bands, hex
 ...
 *   2. machineOK() accepts machine:"all" — buildAtlas() emits it; the engine as ported
 ...
 *      under hashed family names, so the literal families the engine shipped with never matched.
 ...
 */
  ...
  class CortexMap {
    constructor(canvas, data, opts) {
      ...
      // literal families the engine shipped with never resolve — read the real stacks off the CSS variables.
      let sans = "", mono = "";
      try {
    ...
    resize() {
      ...
      this.S.w = r.width; this.S.h = r.height;
      this.cv.width = Math.max(1, Math.round(r.width * this.S.dpr));
      this.cv.height = Math.max(1, Math.round(r.height * this.S.dpr));
    ...
    draw() {
      ...
      if (S.layout === "rings") {
        ...
        for (const key of this.ORDER) {
          ...
          ctx.font = "700 " + Math.min(32, (15 / k) * 1.6) + "px " + this.SANS;
          ctx.lineJoin = "round";
          ctx.strokeStyle = T.halo;
          ctx.lineWidth = 6 / k;
          ctx.strokeText(L.label, 0, -R);
          ctx.globalAlpha = 0.95;
          ctx.fillStyle = L.color;
          ctx.fillText(L.label, 0, -R);
          ctx.globalAlpha = 1;
        }
        ctx.restore();
      }
    ...
    openDetail(n) {
      ...
      this.onSelect({
        ...
        kv: [["detail", n.detail || "\u2014"], ["status", n.status || "\u2014"]]
          .concat(n.machine && n.machine !== "both" && n.machine !== "all" ? [["machine", n.machine]] : [])
