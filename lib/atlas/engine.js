 ...
 * Graph + heat updates 2026-08-11 (the learning layer reaches the board):
 *  10. data.edges may carry kind/w/why for note→note ties (lib/atlas.ts budgets them). Ties
 *      whose kind is in NOTE_EDGE draw as quadratic chords bowed toward the core, kind mapped
 *      to dash/opacity/width on the memory layer's own accent — no new hues. Unknown kinds
 *      keep the straight steel lines the snapshot edges always had.
 *  11. Memory nodes may carry temp ("hot"/"warm"/"cold", from note_scores) — fill intensity
 *      follows it, hot nodes wear a faint glow, and the drawer gains a temperature row. A node
 *      without temp renders exactly as before, so an absent store changes nothing.
 *  12. openDetail() reports each note tie's kind/weight/evidence (MapSelection.edges) so the
 *      drawer can say WHY two notes are connected; `related` carries the remaining neighbours.
 *  13. Note chords draw in across the first ~1.2s of a mount, strongest first, staggered;
 *      prefers-reduced-motion renders the finished graph from frame one.
 * Client-side only: touches canvas, devicePixelRatio and rAF.
 */

  const TAU = Math.PI * 2;

  /* The note graph's line grammar: ONE hue (the memory layer's own accent), the kind carried
     by dash/opacity/width — the ledger's strongest-claim-first ranking restated as ink. An
     explicit link is solid and assertive; a correction chain is dashed (it marks something
     retracted); shared tags are quiet; co-reading is dotted (observed, not written); lexical
     similarity is the faintest whisper on the board. Keys double as the "this is a note tie"
     test — an edge kind outside this table draws as the straight machine wiring always did. */
  const NOTE_EDGE = {
    link:       { a: 0.50, w: 1.30, dash: null },
    correction: { a: 0.55, w: 1.20, dash: [5, 3] },
    tag:        { a: 0.30, w: 1.00, dash: null },
    coaccess:   { a: 0.34, w: 1.00, dash: [1.5, 2.7] },
    lexical:    { a: 0.15, w: 0.85, dash: null },
  };
  /* tag and coaccess are stored once and mean the same read backwards — same set lib/edges.ts
     keeps; the drawer's direction arrow must not invent an arrow the data does not claim. */
  const SYMMETRIC_KINDS = new Set(["tag", "coaccess"]);
  /* How the drawer captions a tie's weight — the ledger's vocabulary, so the same edge never
     reads "3" on one screen and "3 refs" on another. */
  function tieWeight(kind, w) {
    if (typeof w !== "number") return "";
    if (kind === "link") return w + " ref" + (w === 1 ? "" : "s");
    if (kind === "correction") return w + " marker" + (w === 1 ? "" : "s");
    if (kind === "tag") return w + " shared tag" + (w === 1 ? "" : "s");
    if (kind === "coaccess") return w + " co-read";
    if (kind === "lexical") return "bm25 " + w;
    return String(w);
  }
  ...
  const THEMES = {
    ...
    light: {
      /* Retuned 2026-09-01 to the console's paper world: warm paper knock, true ink labels,
         hairline ink edges, and signal orange where the board runs hot — a selection's edges
         and a search match are actions on the document, and orange is the action color. */
      knock: "#f0efed", label: "#111315", labelDim: "#5f666d",
      halo: "rgba(240,239,237,.94)", grid: "rgba(17,19,21,.05)",
      edge: "rgba(17,19,21,.17)", edgeBulk: "rgba(17,19,21,.05)",
      edgeDim: "rgba(17,19,21,.04)", edgeHot: "rgba(216,80,15,.78)",
      core: "#111315", coreGlow: "17,19,21", spoke: "rgba(17,19,21,.15)", match: "#d8500f",
      chip: "#f7f6f4", chipRing: "rgba(17,19,21,.38)",
      layers: { applications: "#3a3f44", routines: "#7c848c", memory: "#0e8ba1", skills: "#5f666d" }
    }
  };
  ...
  const TRIM = { vercel: 0.86, linear: 0.94, supabase: 0.9, claude: 0.94, anthropic: 0.9, apple: 0.92, markdown: 1.04, terminal: 1.02, bash: 0.98, plug: 0.94, cortex: 1.02 };
  /* Routine and skill groups get a glyph by what they ARE, not who made them. */
  const GROUP_GLYPH = {
    agents: "agent", commands: "command", hooks: "hook",
    user: "user", plugin: "plug", anthropic: "anthropic", repo: "github"
  };
  const ALIASES = [
  ...
  class CortexMap {
    constructor(canvas, data, opts) {
      ...
      this.EDGES = data.edges
        .map((e) => ({ s: this.BY.get(e.source), t: this.BY.get(e.target), kind: e.kind, w: e.w, why: e.why }))
        .filter((e) => e.s && e.t);
      ...
      /* Per-node index of the note graph, for the drawer: which ties, of what kind, on what
         evidence. Machine wiring stays out — the drawer's `related` list already carries it. */
      this.EIDX = new Map();
      this.NEDGES = 0;
      const tie = (id, row) => {
        if (!this.EIDX.has(id)) this.EIDX.set(id, []);
        this.EIDX.get(id).push(row);
      };
      for (const e of this.EDGES) {
        if (!NOTE_EDGE[e.kind]) continue;
        this.NEDGES++;
        const both = SYMMETRIC_KINDS.has(e.kind);
        tie(e.s.id, { id: e.t.id, kind: e.kind, w: e.w, why: e.why, dir: both ? "both" : "out" });
        tie(e.t.id, { id: e.s.id, kind: e.kind, w: e.w, why: e.why, dir: both ? "both" : "in" });
      }

      /* One read at mount, mirroring the board component's own check: whoever asked for no
         motion gets the finished graph on the first frame instead of a draw-in. */
      let still = false;
      try { still = matchMedia("(prefers-reduced-motion: reduce)").matches; } catch { /* headless */ }
      this.STILL = still;
    ...
    draw() {
      ...
      if (S.edges) {
        const mem = (this.LAYER.memory || {}).color || T.core;
        let ni = -1;
        for (const e of this.EDGES) {
          const style = NOTE_EDGE[e.kind];
          // The stagger index advances for every note tie, visible or not, so a tie keeps its
          // place in the draw-in even while its group is collapsed.
          if (style) ni++;
          const s = e.s.__proxy || e.s, t = e.t.__proxy || e.t;
          if (!s.__vis || !t.__vis || s === t) continue;
          ...
          if (!style) {
            // The machine wiring, exactly as it always drew.
          ...
          /* Draw-in: each chord grows from its source across the first ~1.2s, strongest first
             (the budget ships them strongest-first). Reduced motion starts at p = 1. */
          let p = 1;
          if (!this.STILL) {
            const start = 180 + (ni / Math.max(1, this.NEDGES - 1)) * 700;
            p = Math.min(1, Math.max(0, (S.t - start) / 420));
            if (p <= 0) continue;
            p = p * (2 - p);
          }

          /* The chord: a quadratic bowed toward the core, so the graph reads as structure
             inside the ring rather than a lattice crossing every label. */
          const bx = (s.x + t.x) / 2 * 0.72, by = (s.y + t.y) / 2 * 0.72;
          let c1x = bx, c1y = by, ex = t.x, ey = t.y;
          if (p < 1) {
            // de Casteljau at p — the partial curve retraces the exact final path.
            const ax = s.x + (bx - s.x) * p, ay = s.y + (by - s.y) * p;
            const qx = bx + (t.x - bx) * p, qy = by + (t.y - by) * p;
            c1x = ax; c1y = ay;
            ex = ax + (qx - ax) * p; ey = ay + (qy - ay) * p;
          }

          const ga = ctx.globalAlpha;
          ctx.globalAlpha = hot ? 0.9 : dim ? 0.06 : style.a;
          ctx.strokeStyle = mem;
          // Weight thickens a tie slightly; the ramp is clamped so a 40-window coaccess pair
          // cannot draw a cable across the board.
          const wk = typeof e.w === "number" ? Math.min(0.7, Math.max(0, (e.w - 1) * 0.18)) : 0;
          ctx.lineWidth = (hot ? 1.9 : style.w + wk) / k;
          if (style.dash) ctx.setLineDash(style.dash.map((d) => d / k));
          ctx.beginPath(); ctx.moveTo(s.x, s.y); ctx.quadraticCurveTo(c1x, c1y, ex, ey); ctx.stroke();
      ...
      for (const n of this.VIS) {
        ...
        if (n.isHub || n.isCenter) {
          ...
          /* The centre wears the Anthropic mark in signal orange over the ink hex — the one
             brand mark drawn at the middle of the board, same simple-icons grid as the rest. */
          if (n.isCenter && n.r * k > 6) {
            const box = n.r * 1.1;
            const px = Math.max(16, Math.min(192, Math.ceil((box * k * this.S.dpr) / 8) * 8));
            ctx.drawImage(sprite("anthropic", px, "#d8500f"), n.x - box / 2, n.y - box / 2, box, box);
          }
        } else {
          const st = n.status, glyph = this.iconFor(n);
          const fav = n.layer === "applications" && n.r * k > 5 ? favicon(glyph) : null;
          /* Heat: temperature is brightness on the live ring — hot glows, warm is the ring's
             normal self, cold recedes to an outline. Same accent, three intensities; a node
             without temp (store absent, machine rings) renders exactly as before. */
          if (n.temp === "hot" && !dim) {
            const rr = n.r * 2.6;
            const g = ctx.createRadialGradient(n.x, n.y, 0, n.x, n.y, rr);
            g.addColorStop(0, L.color + "30"); g.addColorStop(1, L.color + "00");
            ctx.fillStyle = g; ctx.beginPath(); ctx.arc(n.x, n.y, rr, 0, TAU); ctx.fill();
          }
          ...
          if (fav) {
            ...
            ctx.fillStyle = st === "disabled" ? L.color + "12"
              : st === "needs-auth" ? L.color + "33"
              : n.temp === "cold" ? L.color + "3a"
              : n.temp === "warm" ? L.color + "c8" : L.color;
            ctx.fill();
            if (n.temp === "cold") {
              // The outline keeps a cold note findable — dimmed is not deleted.
              ctx.strokeStyle = L.color + "66"; ctx.lineWidth = 1 / k; ctx.stroke();
            }
          }
    ...
    openDetail(n) {
      ...
      /* The note graph's ties, evidence attached \u2014 the drawer shows WHY a line exists, not
         just that it does. Neighbours the graph does not explain stay in `related`. */
      const ties = (this.EIDX.get(n.id) || []).map((e) => {
        const m = this.BY.get(e.id);
        return {
          id: e.id, label: m ? m.label : e.id, kind: e.kind, dir: e.dir,
          weight: tieWeight(e.kind, e.w), why: e.why || ""
        };
      });
      const explained = new Set(ties.map((t) => t.id));
      const rel = [...(this.ADJ.get(n.id) || new Set())]
        .filter((id) => !explained.has(id))
        .map((id) => this.BY.get(id)).filter(Boolean)
        .map((m) => ({ id: m.id, label: m.label, layer: (this.LAYER[m.layer] || {}).label || m.layer }));
      this.onSelect({
        id: n.id, label: n.label, layerLabel: L.label, layerColor: L.color, layerKey: n.layer,
        group: n.group,
        description: n.description || "\u2014",
        kv: [["detail", n.detail || "\u2014"], ["status", n.status || "\u2014"]]
          .concat(n.temp ? [["temperature", n.temp + (typeof n.reads === "number" ? " \u00b7 " + n.reads + " reads" : "")]] : [])
