...
export function registerTools(server: McpServer): void {
  server.registerTool(
    "brain_ask",
    {
      ...
      description:
        "Optional verified ask: answer a question via a server-side Anthropic reader, with a deterministic citation stamp. Prefer brain_corpus when THIS conversation can read the notes itself (Cursor, GPT, Gemini, Claude Code, and most MCP clients) — that path needs no reader key. Use brain_ask when you want a VERIFIED/SUPERSEDED/… stamp without loading the corpus into context, or when ANTHROPIC_API_KEY is configured for that path. How it works: the whole live corpus is fetched in ONE request, a keyword pass picks the ~10 most likely notes (a cost optimisation with ~99% recall — it never decides the answer), and a reader model reads those notes and answers from them. The cited quote is then checked deterministically against the file. If the corpus does not contain the answer it says NOT IN BRAIN rather than guessing. Network egress: api.github.com and codeload.github.com for the corpus, plus ONE call to Anthropic carrying the question and the selected notes. Set full=true to read the entire brain instead of a shortlist.",
      inputSchema: {
        question: z
          .string()
          .min(1)
  ...
  server.registerTool(
    "brain_corpus",
    {
      ...
      description:
        "PREFER THIS for factual work. Return the brain's notes as text so THIS conversation can read them directly — the straight coded read path, with no separate reader model and no Anthropic egress. Use for questions about the operator, projects, machines, or decisions whenever the calling model can hold the notes (Cursor, GPT, Gemini, Claude Code, etc.). With no question it returns the ENTIRE live corpus — the result reports its own size, and it is a large fraction of most context windows, so pass a question unless you genuinely want everything. With a question it returns the most relevant notes only. No model is called and nothing leaves the brain's own storage. Pair with brain_write / brain_capture for writes; use brain_ask only when you specifically need a server-side verified stamp.",
      inputSchema: {
        question: z
          .string()
          .max(2000)
