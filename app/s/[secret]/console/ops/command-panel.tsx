...
export function JobDetail({ job,onAcknowledge,onRecover,onReconcile,onLogs,output }: Readonly<{ job: ConsoleJob;onAcknowledge?:()=>void;onRecover?:()=>void;onReconcile?:()=>void;onLogs?:()=>void;output?:string }>) {
  return <div className="opsJobDetail">
    ...
    {onRecover&&<><p className="opsErr">{job.state==="queued"
      ...
      : "This command may have lost its worker's reply. Marking it uncertain records that and nothing more — it does not cancel the run and does not mean it failed. Other migrations and deploys stay blocked until the run is looked up or acknowledged as unresolved."}</p><button type="button" className="opsAct" onClick={onRecover}>Mark lost response uncertain</button></>}
    {(onReconcile||onLogs)&&<div className="opsCommandControls">{onReconcile&&<button type="button" className="opsAct" onClick={onReconcile}>Retry status lookup · attach verified result</button>}{onLogs&&<button type="button" className="opsAct" onClick={onLogs}>Read safe log excerpt</button>}</div>}
...
function ProviderCommandControls({onJob,settingsRoot}:Readonly<{onJob:(job:ConsoleJob)=>void;settingsRoot:string}>){
  ...
  return <div className="opsProviderControls">{catalogState==="loading"&&<p role="status" className="opsLensNote">Loading provider readiness…</p>}{catalogState==="unavailable"&&<div><p role="status" className="opsErr">Provider readiness unavailable.</p><button type="button" className="opsAct" onClick={()=>void loadCatalog()}>Retry readiness</button></div>}{providers.map(provider=><div key={provider.operation}><button type="button" className="opsAct" disabled={busy||catalogState!=="ready"||!provider.configured} onClick={()=>void prepare(provider.operation)}>Prepare {provider.operation}</button>{!provider.configured&&<a className="opsAct" href={`${settingsRoot}#${provider.setupRequirement?operationsRequirementAnchor(provider.setupRequirement):"setOperations"}`}>Set up permissions</a>}<span className="opsLensNote">{provider.detail}</span></div>)}{status&&<p className="opsErr" role="status">{status}</p>}<p className="opsLensNote">{catalogState==="ready"&&providers.some(provider=>!provider.configured)?"Unavailable commands need one-time hosting permissions configured outside Cortex. The service forms in Settings do not grant those permissions. ":""}A key being present does not prove it works or has the rights it needs. Nothing here runs tests or deploys on its own — every run waits for your confirmation.</p></div>;
}
...
export function CommandPanel({ secret }: Readonly<{ secret: string }>) {
  ...
  return <section className="opsCommands" aria-labelledby="ops-commands">
    ...
    <div className="opsCommandControls"><button type="button" className="opsAct opsActPrimary" onClick={() => void run()} disabled={busy}>{busy ? "Running diagnostics" : "Run diagnostics"}</button><button type="button" className="opsAct" onClick={() => void refresh()} disabled={busy}>Refresh</button><span className="opsLensNote">Checks, migrations and deploy require explicit provider configuration; no dispatch occurs without it.</span></div>
    ...
    <div className={`opsJobScroll${settled ? "" : " opsJobScroll-pending"}`}>
      ...
      {cursor&&<button type="button" className="opsAct" onClick={()=>void load(cursor,true)}>Load older receipts</button>}
    </div>
