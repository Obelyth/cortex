...
export function WorkingStateEditor({project="",item,onSaved}:{project?:string;item?:WorkingItem;onSaved:()=>void}) {
  ...
  return <form className={styles.form} onSubmit={e=>{e.preventDefault();void save();}}>
    ...
    {review&&<section>
      ...
      {review.status==="open"?<button type="button" className={styles.button} onClick={()=>{setCurrent(review);setReview(null);setError(null);setReplaceBody(false);setReplaceProject(false);}}><span className="inkSweep" aria-hidden="true" />Use refreshed version; keep my draft</button>:<p className={styles.note}>This item is no longer open. Your draft remains visible above; it cannot overwrite filed or aged history.</p>}
    </section>}
    <div className={styles.actions}>
      {!saved&&<button type="submit" className={`${styles.button} ${styles.primary}`} disabled={disabled||needsRefresh||Boolean(review)||held}><span className="inkSweep" aria-hidden="true" />{busy?"Working…":pendingAdd.current?"Retry original save":current?"Save changes":"Add working state"}</button>}
      {current&&!saved&&<button type="button" className={styles.button} disabled={busy} onClick={()=>void refresh()}><span className="inkSweep" aria-hidden="true" />Refresh item</button>}
      {current?.status==="open"&&!saved&&!dropConfirm&&<button type="button" className={styles.button} disabled={disabled||needsRefresh||Boolean(review)} onClick={()=>setDropConfirm(true)}><span className="inkSweep" aria-hidden="true" />Drop item…</button>}
      {dropConfirm&&<><p className={styles.note}>Age this item out of working context? Its history stays in the store.</p><button type="button" className={styles.button} disabled={disabled||needsRefresh||Boolean(review)} onClick={()=>void save(true)}><span className="inkSweep" aria-hidden="true" />Confirm age out</button><button type="button" className={styles.button} onClick={()=>setDropConfirm(false)}><span className="inkSweep" aria-hidden="true" />Keep item</button></>}
      <button type="button" className={styles.button} disabled={busy} onClick={lens.close}><span className="inkSweep" aria-hidden="true" />{saved?"Done":"Close editor"}</button>
    </div>
  </form>;
}
