...
let calls: string;
beforeEach(() => {
  scratch = mkdtempSync(join(tmpdir(), "cortex-ingest-test-"));
  ...
  writeFileSync(join(scratch, "gh"), `#!${process.execPath}\nconst fs=require('node:fs');fs.writeFileSync(process.env.INGEST_TEST_CALLS,JSON.stringify({args:process.argv.slice(2),input:fs.readFileSync(0,'utf8')}));`, { mode: 0o700 });
});
afterEach(() => rmSync(scratch, { recursive: true, force: true }));

function ingest(repo: string, commit = true, viaEnvironment = false) {
  return spawnSync(process.execPath, [resolve("scripts/ingest.mjs"), "--from", notes,
    ...(viaEnvironment ? [] : ["--repo", repo]), ...(commit ? ["--commit"] : [])], {
    encoding: "utf8", env: { ...process.env, PATH: `${scratch}:${process.env.PATH}`, BRAIN_REPO: repo, INGEST_TEST_CALLS: calls },
  });
}
