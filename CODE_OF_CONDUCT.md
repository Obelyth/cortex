...
Cortex is a memory server. Issues, discussions, and pull requests routinely involve people
describing how they store personal notes, and sometimes pasting fragments of them.

**Treat anything another person shares from their own brain as theirs.** Do not quote it onward,
do not reconstruct it from an issue thread, and do not press anyone for the contents of a note in
order to reproduce a bug — a redacted repro is always acceptable here, and asking for one is never
a burden. If you believe a report has accidentally disclosed something private, say so privately
using the contact below rather than replying in the thread; a reply quoting the mistake publishes
it a second time.
...
community leaders responsible for enforcement at **support@obelyth.cloud**.
